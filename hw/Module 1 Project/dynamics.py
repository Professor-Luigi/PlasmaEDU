...
# Boris Bunemann
bb = BB(time, X0, q, m, efield.e_interp, Bfield, bounds=[(0,R-wall_thickness), (-2*np.pi,2*np.pi), (0,trap_length)], coords='cylindrical', deltas=[dr, 0, dz], save_X_data=False, path=__file__)
X_nc = bb.X_nc
...
ax.set_xlabel('x (m)')
ax.set_ylabel('y(m)')
fig.savefig('current_xy_zoomed.png')
